...
- **Bodies live once.** `commonTest`'s `Common*Tests` classes hold the test bodies;
  ...
  a runner must be added to all **three** delegators — 223 shared bodies run in each. The Android ones
  ...
  The 223 is 217 delegated bodies plus its 6.)
- **The counts are not symmetric, and the asymmetry is deliberate.** 223 shared bodies per facade,
  so 446 across the two tested facades (`jvmTest`, `macosArm64Test`), and `androidHostTest` runs
  those 223 **plus 6 Android-only tests** — `AndroidMulticastLockTests`, the one class in that
  source set that is not a delegator, because `Iroh4kAndroid.multicastLock` is `androidMain` code
  over `WifiManager` and there is no other facade to hold to the same behaviour. 229 on Android,
  675 host tests in all. Something that exists only on Android belongs in a class of its own there,
...
- **`androidDeviceTest` is deliberately not a fourth delegator.** It runs on a device or emulator
  ...
  `test` source-set tree. Everything these tests found was invisible to all 675 host tests: a
