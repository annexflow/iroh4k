...
[iroh-ffi](https://github.com/n0-computer/iroh-ffi) is upstream's own binding. On the JVM only, it
may well be what you want. iroh4k exists because of what happens when you are not.
...
macOS, Linux or Windows Kotlin/Native target, and there cannot be one without a different strategy.
iroh4k is that strategy: one Rust crate built twice — a `staticlib` linked into Kotlin/Native through
cinterop, and a `cdylib` reached from the JVM and Android through hand-written JNI. Both facades call
the same core through the same codec, and the same 216 shared test bodies run against each.
...
- **`suspend` and `Flow` are real.** Async Rust operations are registered in an op table and resumed
  ...
  across as cold flows, with the latest-value ones coalescing and the event one reporting a `Lagged`
  element rather than dropping transitions silently.
...
- **Addresses are modelled faithfully.** iroh's `EndpointAddr` holds a *set* of `TransportAddr`s —
  ...
  iroh-ffi flattens that to `{ id, relay_url: String?, addresses: List<String> }`, dropping every
  `Custom` address and every relay URL after the first. iroh4k carries the set, plus an `Unknown`
  variant so a newer iroh's address kind is surfaced rather than lost.

- **There are no reverse callbacks anywhere.** iroh-ffi models `ProtocolHandler` as a UniFFI foreign
  trait, so Rust calls back into Kotlin — on the JVM that means a `JNI_OnLoad`, a cached `JavaVM` and
  an `AttachCurrentThread` on every tokio worker that runs a handler. iroh4k's `Router` is Kotlin over
  `Endpoint.acceptNext()`, so Rust never calls into Kotlin at all.
...
What iroh4k does *not* have is upstream's blessing or upstream's release cadence. It is hand-written,
which is why it is specific about coverage.
