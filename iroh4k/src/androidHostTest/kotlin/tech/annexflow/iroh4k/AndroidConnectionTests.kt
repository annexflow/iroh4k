...
/**
 * Exercises the connection surface from the Android target: the same shared
 * `CommonConnectionTests` bodies as `JvmConnectionTests`, run under Robolectric on the host JVM
 * against `androidMain`'s loader.
 */
@RunWith(RobolectricTestRunner::class)
@Config(sdk = [34])
class AndroidConnectionTests {
