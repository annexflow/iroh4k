...
/**
 * Exercises the router from the Android target: the same shared `CommonRouterTests` bodies
 * as `JvmRouterTests`, run under Robolectric on the host JVM against `androidMain`'s loader.
 *
 * The router itself is pure Kotlin, so there is no `expect`/`actual` pair here to pin down — but
 * the accept chain underneath it is, and a continuation or a cancellation that behaves
 * differently on this facade shows up as a router that hangs. See `CommonRouterTests`.
 */
@RunWith(RobolectricTestRunner::class)
@Config(sdk = [34])
class AndroidRouterTests {
